...
import { fetchJobDetails, createBookmark, deleteBookmark } from '@/services/api';
...
const JobDetails = ({ params }) => {
  ...
  const { user, bookmarks, updateBookmarks } = useAuth();
  ...
  const handleBookmarkToggle = async () => {
    ...
    try {
      ...
      if (isBookmarked) {
        console.log('Attempting to delete bookmark for job:', job.id);
        await deleteBookmark(job.id, token);
        updateBookmarks(bookmarks.filter(b => b.id !== job.id));
      } else {
        console.log('Attempting to create bookmark for job:', job.id);
  ...
  return (
    <main className="min-h-screen bg-gray-50 py-8 px-4 font-montserrat">
      <div className="max-w-4xl mx-auto">
        <Link href="/dashboard" className="text-[#25324B] mb-6 inline-flex items-center hover:text-[#1D2939]">
